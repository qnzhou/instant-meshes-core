...
void reorder_mesh(
    MatrixXu& F,
    std::vector<MatrixXf>& V_vec,
    std::vector<MatrixXf>& F_vec,
    const ProgressCallback& progress)
{
    ...
    for (uint32_t i = 0; i < F.cols(); ++i) unprocessed.insert(i);
    ...
    while (!unprocessed.empty()) {
        ...
        while (!queue.empty()) {
            ...
            uint32_t f_new = nF++;
            queue.pop();
            for (uint32_t j = 0; j < F_vec.size(); ++j) F_new[j].col(f_new) = F_vec[j].col(f_old);
...
void replicate_vertices(MatrixXu& F, std::vector<MatrixXf>& V)
{
    ...
    for (uint32_t i = 0; i < V.size(); ++i) R[i].resize(V[i].rows(), F.size());

    for (uint32_t i = 0; i < F.size(); ++i) {
        uint32_t& idx = F.data()[i];
        for (uint32_t j = 0; j < V.size(); ++j) R[j].col(i) = V[j].col(idx);
        idx = i;
    }
