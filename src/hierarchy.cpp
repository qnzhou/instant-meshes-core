...
AdjacencyMatrix downsample_graph(
    ...
    const ProgressCallback& progress)
{
    struct Entry
    {
        ...
        float order;
        inline Entry(){};
        inline Entry(uint32_t i, uint32_t j, float order)
            : i(i)
            , j(j)
            , order(order)
        {}
    ...
    tbb::parallel_for(
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                ...
                N_p.col(i) = norm > RCPOVERFLOW ? Vector3f(normal / norm) : Vector3f::UnitX();
                A_p[i] = surfaceArea;
    ...
    for (uint32_t i = 0; i < neighborhoodSize.size(); ++i) adj_p[i] = links + neighborhoodSize[i];
...
void generate_graph_coloring_deterministic(
    ...
    const ProgressCallback& progress)
{
    if (progress) progress("Graph coloring", 0.0f);
    phases.clear();
    cout << "    Coloring .. ";
    cout.flush();
    Timer<> timer;
    ...
    cout << "done. (" << phases.size() << " colors, took " << timeString(timer.value()) << ")"
         << endl;
}

void generate_graph_coloring(
    const AdjacencyMatrix& adj,
    uint32_t size,
    std::vector<std::vector<uint32_t>>& phases,
    const ProgressCallback& progress)
...
MultiResolutionHierarchy::MultiResolutionHierarchy()
{
    if (sizeof(Link) != 12)
        throw std::runtime_error(
            "Adjacency matrix entries are not packed! Investigate compiler settings.");
...
void MultiResolutionHierarchy::build(bool deterministic, const ProgressCallback& progress)
{
    ...
    for (int i = 0; i < MAX_DEPTH; ++i) {
        ...
        AdjacencyMatrix adj_p = downsample_graph(
            mAdj[i],
            mV[i],
            mN[i],
            mA[i],
...
void init_random_tangent(const MatrixXf& N, MatrixXf& Q)
{
    ...
    tbb::parallel_for(
        tbb::blocked_range<uint32_t>(0u, (uint32_t)N.cols()),
        [&](const tbb::blocked_range<uint32_t>& range) {
            pcg32 rng;
            rng.advance(range.begin());
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
    ...
    tbb::parallel_for(
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            ...
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                Vector3f s, t;
                coordinate_system(N.col(i), s, t);
                float x = rng.nextFloat() * 2.f - 1.f, y = rng.nextFloat() * 2.f - 1.f;
...
{
    if (levels() == 0) return;
    if (mCQ[0].size() == 0) cout << "Allocating memory for constraints .." << endl;
...
void MultiResolutionHierarchy::printStatistics() const
{
    ...
    size_t field_s = 0, V_s = 0, N_s = 0, A_s = 0, adj_s = 0, tree_s = 0, phases_s = 0, cedge_s = 0,
           cvertex_s = 0;
    for (int i = 0; i < levels(); ++i) {
        ...
        phases_s +=
            mPhases[i].size() * sizeof(std::vector<uint32_t>) + mV[i].cols() * sizeof(uint32_t);
    }
    ...
    cout << "    Vertex data         : " << memString(V_s + N_s + A_s)
         << " (level 0: " << memString(sizeInBytes(mV[0]) + sizeInBytes(mN[0]) + sizeInBytes(mA[0]))
         << ")" << endl;
    cout << "    Adjacency matrices  : " << memString(adj_s)
         << " (level 0: " << memString((mAdj[0][mV[0].cols()] - mAdj[0][0]) * sizeof(Link)) << ")"
         << endl;
    ...
    cout << "    Total               : "
         << memString(field_s + V_s + N_s + A_s + adj_s + tree_s + cedge_s + cvertex_s + phases_s)
         << endl;
}
