    ...
    const ProgressCallback& progress)
{
    ...
    if (F.size() != 0) {
            ...
            for (uint32_t f = range.begin(); f != range.end(); ++f) {
                ...
                for (int i = 0; i < 3; ++i) {
                    ...
                    stats.mMaximumEdgeLength =
                        std::max(stats.mMaximumEdgeLength, (double)edge_length);
        ...
        auto reduce = [](MeshStats s0, MeshStats s1) -> MeshStats {
            ...
            result.mAverageEdgeLength = s0.mAverageEdgeLength + s1.mAverageEdgeLength;
            result.mMaximumEdgeLength = std::max(s0.mMaximumEdgeLength, s1.mMaximumEdgeLength);
