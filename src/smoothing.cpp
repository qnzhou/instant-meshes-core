                ...
                for (uint32_t i = range.begin(); i != range.end(); ++i) {
                    ...
                    if (adj_new[i].size() > 0 && !is_crease) {
                        ...
                        for (auto j : adj_new[i]) {
                            centroid += O.col(j);
                            if (withNormals) avgNormal += N.col(j);
                        }
                        if (withNormals) avgNormal += N.col(i);
                        centroid /= adj_new[i].size();
                        Matrix3f cov = Matrix3f::Zero();
                        for (auto j : adj_new[i])
                            cov += (O.col(j) - centroid) * (O.col(j) - centroid).transpose();
                        Vector3f n =
                            cov.jacobiSvd(Eigen::ComputeFullU).matrixU().col(2).normalized();
                        ...
                        if (withNormals) N_prime.col(i) = n;
                    } else {
                        O_prime.col(i) = O.col(i);
                        if (withNormals) N_prime.col(i) = N.col(i);
                    }
                }
            });
        O_prime.swap(O);
        if (withNormals) N_prime.swap(N);
    }
}
