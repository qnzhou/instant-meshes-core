...
struct BVHBuildTask : public tbb::task
{
    ...
    task* execute()
    {
            ...
            [&](const tbb::blocked_range<uint32_t>& range, Bins result) {
                for (uint32_t i = range.begin(); i != range.end(); ++i) {
                    ...
                    Float centroid =
                        pointcloud ? V(axis, f)
                                   : ((1.0f / 3.0f) *
                                      (V(axis, F(0, f)) + V(axis, F(1, f)) + V(axis, F(2, f))));
        ...
        for (int i = Bins::BIN_COUNT - 2; i >= 0; --i) {
            ...
            Float sah_cost =
                2.0f * BVH::T_aabb + tri_factor * (prims_left * bounds_left[i].surfaceArea() +
    ...
    static void
    execute_serially(BVH& bvh, uint32_t node_idx, uint32_t* start, uint32_t* end, uint32_t* temp)
    {
...
void BVH::build(const ProgressCallback& progress)
{
    ...
    cout.precision(4);
    cout << "Compressing BVH node storage to " << 100 * stats.second / (float)mNodes.size()
    ...
    for (int64_t i = stats.second - 1, j = mNodes.size(), skipped = 0; i >= 0; --i) {
        while (mNodes[--j].isUnused()) skipped++;
        BVHNode& new_node = compressed[i];
        new_node = mNodes[j];
        skipped_accum[j] = skipped;

        if (new_node.isInner()) {
            new_node.inner.rightChild = i + new_node.inner.rightChild - j -
    ...
    if (pointcloud) {
        ...
        auto map = [&](const tbb::blocked_range<uint32_t>& range, double radius_sum) -> double {
            ...
            for (uint32_t i = range.begin(); i < range.end(); ++i) {
                ...
                if (findNearest(mV->col(i), radius) != (uint32_t)-1) radius_sum += radius;
            }
        ...
        mDiskRadius =
            tbb::parallel_deterministic_reduce(range, 0, map, reduce) / (double)range.size();
        mDiskRadius *= 3;
        refitBoundingBoxes();
        cout << "done. (took " << timeString(timer.value()) << ")" << endl;
    }
    ...
    if (mF->size() > 0) {
        while (true) {
            ...
            if (node.isInner()) {
                ...
                Vector2f uv;
                for (uint32_t i = node.start(), end = node.end(); i < end; ++i)
                    if (rayIntersectTri(ray, mIndices[i], t, uv)) return true;
                if (stack_idx == 0) break;
                node_idx = stack[--stack_idx];
                continue;
            }
        ...
        while (true) {
            ...
            if (node.isInner()) {
                ...
                Float t;
                for (uint32_t i = node.start(), end = node.end(); i < end; ++i)
                    if (rayIntersectDisk(ray, mIndices[i], t)) return true;
                if (stack_idx == 0) break;
                node_idx = stack[--stack_idx];
                continue;
            }
    ...
    while (true) {
        ...
        if (node.isInner()) {
            ...
            for (uint32_t i = start; i < end; ++i) {
                ...
                if (mF->size() > 0) {
                    for (int j = 0; j < 3; ++j) pointPos += mV->col((*mF)(j, f));
                    pointPos *= 1.0f / 3.0f;
                } else {
                    pointPos = mV->col(f);
                }
                Float pointDist2 = (pointPos - p).squaredNorm();
                if (pointDist2 < radius2 && (pointDist2 != 0 || includeSelf)) result.push_back(f);
            }
            if (stack_idx == 0) break;
            node_idx = stack[--stack_idx];
            continue;
    ...
    while (true) {
        ...
        if (node.isInner()) {
            ...
            if (distLeft < distRight) {
                node_idx = left;
                if (distRight < radius2) stack[stack_idx++] = right;
            } else {
                node_idx = right;
                if (distLeft < radius2) stack[stack_idx++] = left;
            }
            assert(stack_idx < 64);
        } else {
            uint32_t start = node.leaf.start, end = start + node.leaf.size;
    ...
    while (true) {
        ...
        if (node.isInner()) {
            ...
            for (uint32_t i = start; i < end; ++i) {
                ...
                if (mF->size() > 0) {
                    for (int j = 0; j < 3; ++j) pointNormal += mN->col((*mF)(j, f));
                } else {
...
{
    const Vector3f &p0 = mV->col((*mF)(0, i)), &p1 = mV->col((*mF)(1, i)),
                   &p2 = mV->col((*mF)(2, i));
    ...
    if (tempT < ray.mint || tempT > ray.maxt) return false;
...
bool BVH::rayIntersectDisk(const Ray& ray, uint32_t i, Float& t) const
{
    ...
    if (std::abs(dp) < RCPOVERFLOW) return false;
...
std::pair<Float, uint32_t> BVH::statistics(uint32_t node_idx) const
{
    ...
    if (node.isLeaf()) {
        ...
        Float sahCost =
            2 * BVH::T_aabb + (saLeft * stats_left.first + saRight * stats_right.first) / saCur;
...
void BVH::refitBoundingBoxes(uint32_t node_idx)
{
    ...
    if (node.isLeaf()) {
        for (uint32_t i = node.start(); i < node.end(); ++i) {
            ...
            AABB aabb;
            for (int k = 0; k < 4; ++k)
                aabb.expandBy(p + mDiskRadius * (((k & 1) * 2 - 1) * s + ((k & 2) - 1) * t));
            node.aabb = aabb;
        }
    } else {
