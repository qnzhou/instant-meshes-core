    ...
    const ProgressCallback& progress)
{
    ...
    struct EdgeComp
    {
        bool operator()(const Edge& u, const Edge& v) const { return u.second < v.second; }
    };
    ...
    if (progress) progress("Subdividing mesh", 0.0f);
    ...
    auto subdiv = [&](const tbb::blocked_range<uint32_t>& range) {
        for (uint32_t i = range.begin(); i < range.end(); ++i) {
            ...
            if (length > maxLength) {
                ...
                if (other == INVALID || other > i) queue.push(Edge(i, length));
            }
        }
   ...
   */
    ...
    while (!queue.empty()) {
        ...
        if ((V.col(v0) - V.col(v1)).squaredNorm() != edge.second) continue;
        ...
        /* Update E2E */
        const uint32_t e0p = E2E[dedge_prev_3(e0)], e0n = E2E[dedge_next_3(e0)];
...
#define sE2E(a, b) \
    ...
    if (b != INVALID) E2E[b] = a;
        ...
        if (is_boundary) {
            ...
            const uint32_t e1p = E2E[dedge_prev_3(e1)], e1n = E2E[dedge_next_3(e1)];
            sE2E(3 * f0 + 2, 3 * f1 + 0);
            sE2E(3 * f1 + 1, e1n);
            sE2E(3 * f1 + 2, 3 * f2 + 0);
            sE2E(3 * f2 + 1, e1p);
...
#undef sE2E
        ...
        auto schedule = [&](uint32_t f) {
            for (int i = 0; i < 3; ++i) {
                ...
                if (length > maxLength) queue.push(Edge(f * 3 + i, length));
            }
        };
    ...
    cout << "done. (split " << nSplit << " edges, took " << timeString(timer.value())
         << ", new V=" << V.cols() << ", F=" << F.cols() << ", took " << timeString(timer.value())
         << ")" << endl;
}
