...
extern void generate_smooth_normals(
    const MatrixXu& F,
    const MatrixXf& V,
    MatrixXf& N,
    bool deterministic,
    const ProgressCallback& progress)
{
    ...
    auto map = [&](const tbb::blocked_range<uint32_t>& range) {
        for (uint32_t f = range.begin(); f != range.end(); ++f) {
            ...
            for (int i = 0; i < 3; ++i) {
                Vector3f v0 = V.col(F(i, f)), v1 = V.col(F((i + 1) % 3, f)),
                         v2 = V.col(F((i + 2) % 3, f)), d0 = v1 - v0, d1 = v2 - v0;
                ...
                Float angle =
                    fast_acos(d0.dot(d1) / std::sqrt(d0.squaredNorm() * d1.squaredNorm()));
                for (uint32_t k = 0; k < 3; ++k) atomicAdd(&N.coeffRef(k, F(i, f)), fn[k] * angle);
            }
        }
    ...
    cout << "done. (";
    if (badFaces > 0) cout << badFaces << " degenerate faces, ";
    ...
    const ProgressCallback& progress)
{
    ...
    tbb::parallel_for(
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                ...
                if (nonManifold[i] || edge == INVALID) continue;
                ...
                do {
                    ...
                    if (Nf.col(edge / 3).dot(Nf.col(nextEdge / 3)) < dpThreshold) {
                        nCreaseEdges++;
                        if (creaseEdge == INVALID || creaseEdge < nextEdge) creaseEdge = nextEdge;
                    }
                    edge = nextEdge;
                } while (edge != stop);

                if (creaseEdge != INVALID) {
                    if (!is_boundary) V2E[i] = creaseEdge;
    ...
    tbb::parallel_for(
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                ...
                do {
                    uint32_t idx = edge % 3;
                    if (vertexID != i) F(idx, edge / 3) = vertexID;
                    ...
                    if (opp == INVALID) {
                        Float norm = normal.norm();
                        N.col(vertexID) =
                            norm > RCPOVERFLOW ? Vector3f(normal / norm) : Vector3f::UnitX();
                        break;
                    }
    ...
    cout << "done. (";
    if (badFaces > 0) cout << badFaces << " degenerate faces, ";
    if (creaseVert > 0) cout << creaseVert << " crease vertices, ";
    cout << "took " << timeString(timer.value()) << ")" << endl;
}
    ...
    const ProgressCallback& progress)
{
    ...
    tbb::parallel_for(
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                ...
                if (creaseEdge != INVALID) {
                    if (!boundary[i]) V2E[i] = creaseEdge;
                }
            }
            SHOW_PROGRESS_RANGE(range, V.cols(), "Computing vertex & crease normals (2/3)");
        });
    ...
    cout << "done. (";
    if (badFaces > 0) cout << badFaces << " degenerate faces, ";
    if (!creases.empty()) cout << creases.size() << " crease vertices, ";
    cout << "took " << timeString(timer.value()) << ")" << endl;
}
