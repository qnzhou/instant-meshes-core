    ...
    const ProgressCallback& progress)
{
    ...
    for (uint32_t i = 0; i < neighborhoodSize.size(); ++i) adj[i] = links + neighborhoodSize[i];
...
AdjacencyMatrix generate_adjacency_matrix_cotan(
    const MatrixXu& F,
    const MatrixXf& V,
    const VectorXu& V2E,
    const VectorXu& E2E,
    ...
    tbb::parallel_for(
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i != range.end(); ++i) {
                ...
                int it = 0;
                do {
                    uint32_t f = edge / 3, curr_idx = edge % 3, next_idx = (curr_idx + 1) % 3,
                             prev_idx = (curr_idx + 2) % 3;

                    if (it == 0) {
                        Vector3f p = V.col(F(next_idx, f)), d0 = V.col(F(prev_idx, f)) - p,
                                 d1 = V.col(F(curr_idx, f)) - p;
                        Float cot_weight = 0.0f, sin_alpha = d0.cross(d1).norm();
                        if (sin_alpha > RCPOVERFLOW) cot_weight = d0.dot(d1) / sin_alpha;
                    ...
                    uint32_t opp = E2E[edge], next = dedge_next_3(opp);
                    if (opp == INVALID || next != stop) {
                        Vector3f p = V.col(F(prev_idx, f)), d0 = V.col(F(curr_idx, f)) - p,
    ...
    const ProgressCallback& progress)
{
    ...
    tbb::parallel_for(
        ...
        [&](const tbb::blocked_range<uint32_t>& range) {
            for (uint32_t i = range.begin(); i < range.end(); ++i) {
                uint32_t* adj_set_i = adj_sets + (size_t)i * (size_t)knn_points;
                if (adj_size[i] == INVALID) continue;
                ...
                for (uint32_t j = 0; j < knn_points; ++j) {
                    ...
                    if (!found) adj[k][atomicAdd(&adj_offset.coeffRef(k), 1) - 1] = Link(i);
                }
            }
        });
    ...
    /* Use a heuristic to estimate some useful quantities for point clouds (this
       ...
       reasonably accurate) */
    stats.mSurfaceArea =
        kPi * stats.mAverageEdgeLength * stats.mAverageEdgeLength * 0.5f * V.cols();
