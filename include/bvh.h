...
struct BVHNode
{
    ...
    inline bool isUnused() const { return inner.unused == 0 && inner.rightChild == 0; }

    inline uint32_t start() const { return leaf.start; }

    inline uint32_t end() const { return leaf.start + leaf.size; }
};
...
public:
    ...
    void setData(const MatrixXu* F, const MatrixXf* V, const MatrixXf* N)
    {
        mF = F;
        mV = V;
        mN = N;
