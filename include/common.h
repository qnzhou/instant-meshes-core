...
#define SHOW_PROGRESS(i, maxval, text) \
    if (progress && (i % PROGRESS_BLKSIZE) == 0) progress(text, -PROGRESS_BLKSIZE / (Float)maxval)

#define PROGRESS_SHIFT 18u
#define SHOW_PROGRESS_RANGE(range, maxval, text)                                       \
    if (progress && range.begin() > 0) {                                               \
        uint32_t nUpdates =                                                            \
            (range.end() >> PROGRESS_SHIFT) - ((range.begin() - 1) >> PROGRESS_SHIFT); \
        if (nUpdates > 0) {                                                            \
...
inline T union_cast(const U& val)
{
    union {
        U u;
        T t;
...
public:
    ...
    void lock()
    {
        ...
        while (ticket != counter) cvar.wait(acquire);
    }
    void unlock()
    {
        std::unique_lock<std::mutex> acquire(cvar_lock);
    ...
    bool include_empty = false)
{
    ...
    while (std::getline(ss, item, delim))
        if (!item.empty() || include_empty) elems.push_back(item);
    return elems;
}
